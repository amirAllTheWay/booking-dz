...
class AdminPage extends Component{
    ...
    componentWillUnmount() {
        if (this.props.offerManagementStatus != null) {
            this.props.onAdminPageUnMounting();
        }
    }
    ...
    onSubmitNewOfferClicked() {
        ...
        this.props.onAddOfferClicked({isHotOffer: false, offerTitle: this.offerTitle.current.value, departureCity: this.departureCity.current.value, destinationCity: this.destinationCity.current.value,
                                                departureDate: this.state.departureDate.toISOString().split("T")[0], returnDate: this.state.returnDate.toISOString().split("T")[0],
    ...
    render() {
        ...
        return(
            <div className="AdminFormContainer">
                ...
                <Tabs defaultActiveKey="tourismOffers" id="uncontrolled-tab-example">
                    <Tab eventKey="tourismOffers" title="Voyages touristiques">
                        <div>

                            <h3 className="p-4">Ajouter une offre de tourisme</h3>
                            <Form>
                            ...
                            <Col className="Padding" sm={{ span: 10, offset: 1, padding: 3 }}>
                                <Button className="btn-lg" type="submit" onClick={() => this.onSubmitNewOfferClicked()}>Ajouter l'offre</Button>
...
const mapDispatchToProps = dispatch => {
    ...
    return {
        onAddOfferClicked: (offer) => {
            dispatch(actionCreators.addTourismOffer(offer));
        },

        onAdminPageUnMounting: () => {
            dispatch({type: "ADMIN_PAGE_UNMOUNTING"});
        }
